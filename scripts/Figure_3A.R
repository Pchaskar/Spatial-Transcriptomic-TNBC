...
colnames(norm_counts) <-
  gsub("TIL", "sTIL", colnames(norm_counts))
...
mcp_results <-
  res_mcp_counter_all %>% gather(sample, score,-cell_type)
...
mcp_results$sample <-
  factor(mcp_results$sample, levels = unique(mcp_results$sample[order(mcp_results$ct)]))

#Rename MCP counter output
colnames(mcp_results) <-
  c("cell_type", "Samples", "MCP-counter score", "ct")

#Remove monocytes
mcp_results_nomono <-
  mcp_results[!mcp_results$cell_type == 'Monocyte', ]
...
immnue_cells <-
  c(
    ...
    `NK cell` = "NK",
    `Macrophage/Monocyte` = "Monocytic lineage",
    `Myeloid dendritic cell` = "Myeloid dendritic",
    `cytotoxicity score` = "Cytotoxic lymphocytes",
    `Cancer associated fibroblast` = "CAF",
    `Endothelial cell` = "Endothelial Cells"
  )

mcp_results_nomono$immnue_cells <-
