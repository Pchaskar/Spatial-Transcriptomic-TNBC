...
colnames(vsd_matrix_fib) <-
  gsub("TIL", "sTIL", colnames(vsd_matrix_fib))
...
df_out$Cells <-
  sapply(strsplit(as.character(row.names(t(
    vsd_matrix_fib
  ))), "_"), "[[", 2)# group on cell type TC, TIL, Fib
head(df_out)
...
p <-
  p + geom_point(size = 3) + xlab(percentage[1]) + ylab(percentage[2])
p <-
  p + geom_point(size = 3) + theme(
...
pca_ggplot <-
  p + scale_colour_manual(values = c("blue", "brown", "violet"))

tiff(
  "./images/Figure2_part1.tiff",
  width = 4,
  height = 3.5,
  units = 'in',
  res = 600,
...
deg_vsd_matrix <-
  subset(vsd_matrix_NoFib,
         rownames(vsd_matrix_NoFib) %in% rownames(sig_deg))
dim(deg_vsd_matrix)
...
sig_deg_up_20top <-
  head(sig_deg_up[order(sig_deg_up$padj, decreasing = FALSE),], 20)

sig_deg_down <- sig_deg[sig_deg$log2FoldChange < 0,]
sig_deg_down_20top <-
  head(sig_deg_down[order(sig_deg_down$padj, decreasing = FALSE),], 20)
...
ha_de_up = rowAnnotation(link = anno_mark(
  at_up,
  labels = rownames(deg_vsd_matrix_scaled[at_up, ]),
  labels_gp = gpar(fontsize = 8, col =
                     "red")
))

ha_de_down = rowAnnotation(link = anno_mark(
  at_down,
...
ha_top40 = rowAnnotation(link = anno_mark(
  at_sig,
...
#
# # Save figure
# figure1 %>% save_multi_panel_figure(filename = "./images/Figure2.tiff", dpi = 600, compression = "lzw")
#
